...
def collect_results_rmse_from_df(df, patch_type='init'):
    ...
    # 按车距区间 RMSE，以下以 Torch 的预测值来划分区间
    bounds = [0, 20, 40, 60, 80, 999]
    dist_rmse = {}
    # 这里使用每帧 Torch 预测值的最小值作为 lead car 距离参考
...
if __name__ == '__main__':
    csv_file = "results/vid_test1/chunk_3/video1.csv"  # 根据实际路径修改
    df = pd.read_csv(csv_file)
    
    # 分别展示各攻击类型的结果，按照源代码的 print 格式
    for pt in ['optmask', 'randmask', 'fgsm', 'gauss', 'apgd']:
        print_results(df, pt)
    
    # 展示无攻击（No Patch）的 RMSE 结果
    print("### No Patch ###")
