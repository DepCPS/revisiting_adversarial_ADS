...
class AdvTrainDataset(Dataset):
    def __init__(self, clean_dir, adv_dir, model, device):
        ...
        print(f"Find {len(self.adv_image_paths)} adversarial images in {self.adv_dir}.")
        self.model.eval()
        with torch.no_grad():
            for adv_path in self.adv_image_paths:
                ...
                if not os.path.exists(clean_path):
                    print(f"WARNING: {filename} not found in clean image, skipping.")
                    continue
        ...
        if len(self.gt_distances) == 0:
            raise ValueError("No valid samples were found in the dataset, please check the folder path and file name.")
        else:
            print(f"The final dataset contains {len(self.gt_distances)} samples.")
...
# --------------------- Multi-attack dataset (randomly sampling 25% images from each folder) ---------------------
class CombinedAdvTrainDataset(Dataset):
    def __init__(self, clean_dir, attack_dirs, model, device, sample_ratio=0.25):
        ...
        print(f"A total of {len(self.adv_image_paths)} adversarial images are sampled from the folder {attack_dirs}.")
        self.model.eval()
        with torch.no_grad():
            valid_paths = []
            for adv_path in self.adv_image_paths:
        ...
        if len(self.adv_image_paths) == 0:
            raise ValueError("No valid samples were found in the combined dataset, please check the folder path and file name.")
        else:
            print(f"The final combined dataset contains {len(self.adv_image_paths)} samples.")
...
if __name__ == "__main__":
    # results/vid_test1/chunk_3: clean, gauss, fgsm, apgd, opt 
    chunk = 3
    ...
    # Evaluation using images from all 4 attacks
    attack_dirs_eval = [gauss_dir, fgsm_dir, apgd_dir, opt_dir]
    
    # Original model weights (not used for training, only for calculating ground truth)
    ...
    print("\n===== Start with 4 attacks adversarial training =====")
