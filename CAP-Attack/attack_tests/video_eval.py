...
TORCH_WEIGHTS_PATH = '../models/weights/supercombo_torch_weights_adv_combined_new.pth'
        ...
        for file in files:
            if file.endswith('.csv') and file not in blacklist:
                ...
                if init_predictions is None:
                    init_predictions = df['Torch_init_dRel0'].to_numpy()
                    mask_predictions = df[f'Torch_{col_type}_dRel0'].to_numpy()
                else:
                    init_predictions = np.r_[init_predictions, df['Torch_init_dRel0'].to_numpy()]
                    mask_predictions = np.r_[mask_predictions, df[f'Torch_{col_type}_dRel0'].to_numpy()]
