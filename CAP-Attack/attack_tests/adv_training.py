...
def estimate_torch(model, input_imgs, desire=None, traffic_convention=None, recurrent_state=None, device="cpu"):
    ...
    如果输入为 (12,128,256)，则自动 unsqueeze 添加 batch 维度。
...
class AdvTrainDataset(Dataset):
    def __init__(self, clean_dir, adv_dir, model, device):
        ...
        :param adv_dir:   对抗图像文件夹路径（例如选择某一种攻击，如 fgsm）
        ...
        print(f"在 {self.adv_dir} 中找到 {len(self.adv_images)} 张对抗图像。")
        self.model.eval()
        with torch.no_grad():
            for adv_path in self.adv_images:
                filename = os.path.basename(adv_path)  # 例如 "video1_frame1.png"
                clean_path = os.path.join(self.clean_dir, filename)
                if not os.path.exists(clean_path):
                    print(f"警告：干净图像中未找到对应文件 {filename}，跳过。")
                    continue
        ...
        if len(self.adv_image_paths) == 0:
            raise ValueError("未在数据集中找到有效样本，请检查文件夹路径和文件名是否正确。")
        else:
            print(f"最终数据集中包含 {len(self.adv_image_paths)} 个样本。")
    ...
    def __getitem__(self, idx):
        ...
        gt_dist = self.gt_distances[idx]       # numpy 数组，形状 (6,)
...
if __name__ == "__main__":
    # 目录结构示例：results/vid_test1/chunk_3 下包含 clean, gauss, fgsm, apgd, opt 文件夹
    chunk = 3
    ...
    for attack_type, attack_train_dir in attack_dict.items():
        print(f"\n===== 开始使用 {attack_type} 攻击图像进行 adversarial training =====")
